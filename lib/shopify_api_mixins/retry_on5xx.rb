      ...
      loop do
        begin
          ...
          if time_of_first_try < RetryOn5xx.max_wait.ago
            ...
            Rails.logger.error "Recieved too many server errors. Retried #{retries} times over the course of #{actual_wait_time} seconds. Last error was #{e.response.code}: #{e.response.message}:\n#{e.response.body}"
            raise
          end
          code = e.response.code.to_i
          Rails.logger.info "Got a #{code}, will sleep for #{delay} seconds and retry"
          sleep(delay)
          retries += 1
